...
Variant Yield::complete(const Variant **p_args, int p_argcount, Variant::CallError &r_error) {
    if (p_argcount == 0) {
...
void Yield::keep(Ref<Reference> obj) {
    keeped = obj;
}
...
Ref<Yield> Future::yield(Object *obj, const String &signal) {
    ...
    Reference *objref = Object::cast_to<Reference>(obj);
    if (objref != NULL) {
        yields->keep(Ref<Reference>(objref));
    }
    return yields;
}
