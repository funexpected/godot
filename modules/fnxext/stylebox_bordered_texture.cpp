...
void StyleBoxBorderedTexture::set_border_width(Margin p_margin, int p_width) {
	ERR_FAIL_INDEX((int)p_margin, 4);
	border_width[p_margin] = p_width;
	emit_changed();
}

int StyleBoxBorderedTexture::get_border_width(Margin p_margin) const {
...
void StyleBoxBorderedTexture::draw(RID p_canvas_item, const Rect2 &p_rect) const {
    ...
    rect.position.x -= get_expand_margin_size(MARGIN_LEFT);
	rect.position.y -= get_expand_margin_size(MARGIN_TOP);
	rect.size.x += get_expand_margin_size(MARGIN_LEFT) + get_expand_margin_size(MARGIN_RIGHT);
	rect.size.y += get_expand_margin_size(MARGIN_TOP) + get_expand_margin_size(MARGIN_BOTTOM);

    float tx[4] = { rect.position.x, rect.position.x + border_width[MARGIN_LEFT], rect.position.x + rect.size.width - border_width[MARGIN_RIGHT], rect.position.x + rect.size.width };
    float ty[4] = { rect.position.y, rect.position.y + border_width[MARGIN_TOP], rect.position.y + rect.size.height - border_width[MARGIN_BOTTOM], rect.position.y + rect.size.height };
    Vector2 ts[16] = {
    ...
    float sx[4] = { src_rect.position.x, src_rect.position.x + get_margin_size(MARGIN_LEFT), src_rect.position.x + src_rect.size.x - get_margin_size(MARGIN_RIGHT), src_rect.position.x + src_rect.size.x };
    float sy[4] = { src_rect.position.y, src_rect.position.y + get_margin_size(MARGIN_TOP), src_rect.position.y + src_rect.size.y - get_margin_size(MARGIN_BOTTOM), src_rect.position.y + src_rect.size.y };
    Vector2 ss[16] = {
    ...
    for (int i=0; i<11; i++) {
        ...
        VS::get_singleton()->canvas_item_add_texture_rect_region(
            ...
            get_modulate(), 
            false,
            get_normal_map().is_valid() ? get_normal_map()->get_rid() : RID(),
            false
        );
    }
}
