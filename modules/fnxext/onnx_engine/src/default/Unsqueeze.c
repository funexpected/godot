...
static int Unsqueeze_11_init(struct onnx_node_t * n)
{
...
static int Unsqueeze_11_exit(struct onnx_node_t * n)
{
	return 1;
}

static int Unsqueeze_11_reshape(struct onnx_node_t * n)
{
	struct onnx_tensor_t * y = n->outputs[0];
	struct onnx_tensor_t * x = n->inputs[0];
	int64_t * axes = NULL;
	int naxes = onnx_attribute_read_ints(n, "axes", &axes);
	int ndim = x->ndim + naxes;
	int dims[ndim];
	int axis;
	int i, j;
...
static void Unsqueeze_11_operator(struct onnx_node_t * n)
{
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
	char ** px = (char **)x->datas;
...
void resolver_default_op_Unsqueeze(struct onnx_node_t * n)
{
	if(n->opset >= 13)
	{
		...
		case ONNX_TENSOR_TYPE_STRING:
			...
			n->operator_ = Unsqueeze_operator;
			break;
		default:
			break;
		}
	...
	else if(n->opset >= 11)
	{
		...
		case ONNX_TENSOR_TYPE_COMPLEX128:
		case ONNX_TENSOR_TYPE_STRING:
			n->init = Unsqueeze_11_init;
			n->exit = Unsqueeze_11_exit;
			n->reshape = Unsqueeze_11_reshape;
			n->operator_ = Unsqueeze_11_operator;
			break;
		default:
			break;
		}
